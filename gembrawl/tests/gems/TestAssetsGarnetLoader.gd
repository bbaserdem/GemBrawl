...
func _ready():
	# Load the garnet gem model
	var gem_scene = load("res://assets/models/items/gem_garnet.glb")
	if gem_scene:
		var instance = gem_scene.instantiate()
		add_child(instance)
		print("Garnet gem model loaded successfully!")
		...
		# Modify the material to be shiny and purple
		_apply_garnet_material(instance)
	else:
		print("Failed to load garnet gem model")

func _apply_garnet_material(node: Node):
		...
		match shader_mode:
			"faceted":
				...
				if shader:
					...
					# Set shader parameters for garnet
					shader_material.set_shader_parameter("gem_color", Color(0.5, 0.2, 0.7, 1.0))
					shader_material.set_shader_parameter("edge_color", Color(0.3, 0.1, 0.5, 1.0))
					...
					_apply_standard_garnet_material(mesh_instance)
			...
			"outlined":
				...
				if shader:
					...
					# Set shader parameters for garnet
					shader_material.set_shader_parameter("gem_color", Color(0.5, 0.2, 0.7, 1.0))
					shader_material.set_shader_parameter("highlight_color", Color(0.8, 0.5, 1.0, 1.0))
					shader_material.set_shader_parameter("shadow_color", Color(0.2, 0.1, 0.3, 1.0))
					shader_material.set_shader_parameter("outline_color", Color(0.1, 0.05, 0.2, 1.0))
			...
			"crystal":
				...
				if shader:
					...
					# Set shader parameters for garnet
					shader_material.set_shader_parameter("albedo_color", Color(0.5, 0.2, 0.7, 1.0))
					...
					shader_material.set_shader_parameter("rim_color", Color(0.7, 0.4, 1.0, 1.0))
	...
	for child in node.get_children():
		_apply_garnet_material(child)

func _apply_standard_garnet_material(mesh_instance: MeshInstance3D):
	# Create a new shiny garnet material
	var garnet_material = StandardMaterial3D.new()
	
	# Deep purple color - opaque
	garnet_material.albedo_color = Color(0.5, 0.2, 0.7, 1.0)  # Fully opaque
	
	# Make it reflective like a real gem
	garnet_material.metallic = 0.0  # Gems are not metallic
	garnet_material.roughness = 0.1  # Slightly more rough for better facet definition
	garnet_material.specular = 1.0  # Maximum specular reflection
	
	# CRITICAL: Enable vertex shading for flat/angular appearance
	garnet_material.shading_mode = BaseMaterial3D.SHADING_MODE_PER_VERTEX
	garnet_material.vertex_color_use_as_albedo = false
	
	# Disable emission - no glow
	garnet_material.emission_enabled = false
	
	# No transparency - we want to see the edges clearly
	garnet_material.transparency = BaseMaterial3D.TRANSPARENCY_DISABLED
	
	# Disable subsurface scattering
	garnet_material.subsurf_scatter_enabled = false
	
	# Stronger rim lighting to emphasize edges
	garnet_material.rim_enabled = true
	garnet_material.rim = 0.5  # Increased for more edge definition
	garnet_material.rim_tint = 0.2  # Slight color tint for gemstone effect
	
	# No backlight
	garnet_material.backlight_enabled = false
	
	# Enable clearcoat for polished look without glow
	garnet_material.clearcoat_enabled = true
	garnet_material.clearcoat = 1.0  # Maximum clearcoat
	garnet_material.clearcoat_roughness = 0.0  # Perfectly smooth
	
	# Apply the material override
	mesh_instance.material_override = garnet_material
	
	print("Applied angular garnet material with flat shading")
