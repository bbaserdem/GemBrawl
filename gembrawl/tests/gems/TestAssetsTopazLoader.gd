...
func _ready():
	# Load the topaz gem model
	var gem_scene = load("res://assets/models/items/gem_topaz.glb")
	if gem_scene:
		var instance = gem_scene.instantiate()
		add_child(instance)
		print("Topaz gem model loaded successfully!")
		...
		# Modify the material to be shiny and yellow
		_apply_topaz_material(instance)
	else:
		print("Failed to load topaz gem model")

func _apply_topaz_material(node: Node):
		...
		match shader_mode:
			"faceted":
				...
				if shader:
					...
					# Set shader parameters for topaz
					shader_material.set_shader_parameter("gem_color", Color(0.9, 0.7, 0.1, 1.0))
					shader_material.set_shader_parameter("edge_color", Color(0.6, 0.4, 0.0, 1.0))
					...
					_apply_standard_topaz_material(mesh_instance)
			...
			"outlined":
				...
				if shader:
					...
					# Set shader parameters for topaz
					shader_material.set_shader_parameter("gem_color", Color(0.9, 0.7, 0.1, 1.0))
					shader_material.set_shader_parameter("highlight_color", Color(1.0, 0.9, 0.5, 1.0))
					shader_material.set_shader_parameter("shadow_color", Color(0.5, 0.4, 0.05, 1.0))
					shader_material.set_shader_parameter("outline_color", Color(0.3, 0.2, 0.0, 1.0))
			...
			"crystal":
				...
				if shader:
					...
					# Set shader parameters for topaz
					shader_material.set_shader_parameter("albedo_color", Color(0.9, 0.7, 0.1, 1.0))
					...
					shader_material.set_shader_parameter("rim_color", Color(1.0, 0.9, 0.4, 1.0))
	...
	for child in node.get_children():
		_apply_topaz_material(child)

func _apply_standard_topaz_material(mesh_instance: MeshInstance3D):
	# Create a new shiny topaz material
	var topaz_material = StandardMaterial3D.new()
	
	# Bright yellow color - opaque
	topaz_material.albedo_color = Color(0.9, 0.7, 0.1, 1.0)  # Fully opaque
	
	# Make it reflective like a real gem
	topaz_material.metallic = 0.0  # Gems are not metallic
	topaz_material.roughness = 0.1  # Slightly more rough for better facet definition
	topaz_material.specular = 1.0  # Maximum specular reflection
	
	# CRITICAL: Enable vertex shading for flat/angular appearance
	topaz_material.shading_mode = BaseMaterial3D.SHADING_MODE_PER_VERTEX
	topaz_material.vertex_color_use_as_albedo = false
	
	# Disable emission - no glow
	topaz_material.emission_enabled = false
	
	# No transparency - we want to see the edges clearly
	topaz_material.transparency = BaseMaterial3D.TRANSPARENCY_DISABLED
	
	# Disable subsurface scattering
	topaz_material.subsurf_scatter_enabled = false
	
	# Stronger rim lighting to emphasize edges
	topaz_material.rim_enabled = true
	topaz_material.rim = 0.5  # Increased for more edge definition
	topaz_material.rim_tint = 0.2  # Slight color tint for gemstone effect
	
	# No backlight
	topaz_material.backlight_enabled = false
	
	# Enable clearcoat for polished look without glow
	topaz_material.clearcoat_enabled = true
	topaz_material.clearcoat = 1.0  # Maximum clearcoat
	topaz_material.clearcoat_roughness = 0.0  # Perfectly smooth
	
	# Apply the material override
	mesh_instance.material_override = topaz_material
	
	print("Applied angular topaz material with flat shading")
