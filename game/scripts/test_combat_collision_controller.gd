...
@export var player_scene: PackedScene = preload("res://scenes/player/player_3d.tscn")

## Combat test scenes
@export var melee_hitbox_scene: PackedScene = preload("res://scenes/combat/test_melee_hitbox.tscn")
@export var projectile_scene: PackedScene = preload("res://scenes/combat/test_projectile.tscn")
@export var aoe_scene: PackedScene = preload("res://scenes/combat/test_aoe.tscn")
...
var arena: Node3D  # HexArena3D instance
...
func _ready() -> void:
	...
	arena = $HexArena3D
	if not arena:
		push_error("HexArena3D not found!")
...
func _create_hex_spawn_points() -> void:
	...
	for i in spawn_hexes.size():
		...
		var world_pos = HexGrid3D.hex_to_world_3d(spawn_hexes[i])
