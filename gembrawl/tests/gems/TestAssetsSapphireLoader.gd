...
func _ready():
	# Load the sapphire gem model
	var gem_scene = load("res://assets/models/items/gem_sapphire.glb")
	if gem_scene:
		var instance = gem_scene.instantiate()
		add_child(instance)
		print("Sapphire gem model loaded successfully!")
		...
		# Modify the material to be shiny and blue
		_apply_sapphire_material(instance)
	else:
		print("Failed to load sapphire gem model")

func _apply_sapphire_material(node: Node):
		...
		match shader_mode:
			"faceted":
				...
				if shader:
					...
					# Set shader parameters for sapphire
					shader_material.set_shader_parameter("gem_color", Color(0.1, 0.3, 0.8, 1.0))
					shader_material.set_shader_parameter("edge_color", Color(0.0, 0.1, 0.4, 1.0))
					...
					_apply_standard_sapphire_material(mesh_instance)
			...
			"outlined":
				...
				if shader:
					...
					# Set shader parameters for sapphire
					shader_material.set_shader_parameter("gem_color", Color(0.1, 0.3, 0.8, 1.0))
					shader_material.set_shader_parameter("highlight_color", Color(0.5, 0.7, 1.0, 1.0))
					shader_material.set_shader_parameter("shadow_color", Color(0.05, 0.1, 0.3, 1.0))
					shader_material.set_shader_parameter("outline_color", Color(0.0, 0.05, 0.2, 1.0))
			...
			"crystal":
				...
				if shader:
					...
					# Set shader parameters for sapphire
					shader_material.set_shader_parameter("albedo_color", Color(0.1, 0.3, 0.8, 1.0))
					...
					shader_material.set_shader_parameter("rim_color", Color(0.4, 0.6, 1.0, 1.0))
	...
	for child in node.get_children():
		_apply_sapphire_material(child)

func _apply_standard_sapphire_material(mesh_instance: MeshInstance3D):
	# Create a new shiny sapphire material
	var sapphire_material = StandardMaterial3D.new()
	
	# Deep blue color - opaque
	sapphire_material.albedo_color = Color(0.1, 0.3, 0.8, 1.0)  # Fully opaque
	
	# Make it reflective like a real gem
	sapphire_material.metallic = 0.0  # Gems are not metallic
	sapphire_material.roughness = 0.1  # Slightly more rough for better facet definition
	sapphire_material.specular = 1.0  # Maximum specular reflection
	
	# CRITICAL: Enable vertex shading for flat/angular appearance
	sapphire_material.shading_mode = BaseMaterial3D.SHADING_MODE_PER_VERTEX
	sapphire_material.vertex_color_use_as_albedo = false
	
	# Disable emission - no glow
	sapphire_material.emission_enabled = false
	
	# No transparency - we want to see the edges clearly
	sapphire_material.transparency = BaseMaterial3D.TRANSPARENCY_DISABLED
	
	# Disable subsurface scattering
	sapphire_material.subsurf_scatter_enabled = false
	
	# Stronger rim lighting to emphasize edges
	sapphire_material.rim_enabled = true
	sapphire_material.rim = 0.5  # Increased for more edge definition
	sapphire_material.rim_tint = 0.2  # Slight color tint for gemstone effect
	
	# No backlight
	sapphire_material.backlight_enabled = false
	
	# Enable clearcoat for polished look without glow
	sapphire_material.clearcoat_enabled = true
	sapphire_material.clearcoat = 1.0  # Maximum clearcoat
	sapphire_material.clearcoat_roughness = 0.0  # Perfectly smooth
	
	# Apply the material override
	mesh_instance.material_override = sapphire_material
	
	print("Applied angular sapphire material with flat shading")
